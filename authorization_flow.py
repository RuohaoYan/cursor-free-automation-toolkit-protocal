...
def choose_country_interactively(rows: list[PhoneCountry], default_country: PhoneCountry, forced: str = "", provider_label: str = "鎺ョ爜骞冲彴") -> PhoneCountry:
    if forced:
        ...
        if matched:
            print(f"[SMS] 通过 --country 指定国家: {matched.name} (+{matched.dial_code}), {provider_label} 国家ID={matched.hero_sms_country}")
            return matched
        print(f"[SMS] --country={forced} 未匹配当前列表，改用交互选择。")
    while True:
        answer = input(
            f"请选择国家（序号/ISO/{provider_label} 国家ID，回车默认 {default_country.iso_code}）: "
        ).strip()
        if not answer:
...
def choose_operator_interactively(rows: list[OperatorQuote], default_option: OperatorQuote, country: PhoneCountry) -> OperatorQuote:
    while True:
        answer = input(f"请选择 {country.name} 运营商（序号/名称，回车默认 {default_option.label}）: ").strip()
        if not answer:
...
def resolve_sms_operator(provider, service: str, country: PhoneCountry, threshold: int, *, always_prompt: bool = False) -> OperatorQuote:
    ...
    aggregate_option = OperatorQuote("", "any", country.price, country.count, "aggregate quote")
    if not always_prompt and (aggregate_count is None or aggregate_count >= threshold):
        try:
            ...
            operator_options_early = provider.get_operator_quote_options(service, operator_arg)
            auto_operator = pick_operator_quote_by_price_rank(operator_options_early)
            if auto_operator:
                print(
                    f"[SMS] {country.name} 库存 {count_text(aggregate_count)}，"
                    f"自动使用倒数第二低价运营商: {auto_operator.label} (${auto_operator.price})"
                )
                return auto_operator
        except Exception:
            pass
        print(f"[SMS] {country.name} 褰撳墠鑱氬悎搴撳瓨 {count_text(aggregate_count)}锛屼笉瑙﹀彂浜屾杩愯惀鍟嗛€夋嫨")
        return aggregate_option
    try:
        # HeroSMS/Grizzly 浣跨敤 hero_sms_country int锛汧iveSim 鏀寔鐩存帴浼?PhoneCountry
        operator_arg: Any = country if _provider_uses_country_slug(provider) else country.hero_sms_country
        operator_options = provider.get_operator_quote_options(service, operator_arg)
    except Exception as exc:
        print(f"[SMS] 获取 {country.name} 运营商列表失败，使用默认 any: {exc}")
        return aggregate_option
    if always_prompt and not operator_options:
        print(f"[SMS] {country.name} 暂无可选运营商明细，使用默认 any")
        return aggregate_option
    rows = [aggregate_option, *operator_options]
    auto_operator = pick_operator_quote_by_price_rank(operator_options)
    if auto_operator and not always_prompt:
        print(
            f"[SMS] {country.name} 自动使用倒数第二低价运营商: "
            f"{auto_operator.label} (${auto_operator.price})"
        )
        return auto_operator
    if always_prompt and not (sys.stdin.isatty() and sys.stdout.isatty()):
        print(f"[SMS] 褰撳墠涓嶆槸浜や簰缁堢锛岃嚜鍔ㄤ娇鐢ㄩ粯璁ゆ湇鍔″晢: {aggregate_option.label}")
        return aggregate_option
    if always_prompt:
        print(f"[SMS] {country.name} 当前库存 {count_text(aggregate_count)}，请选择运营商（回车默认 any）")
    else:
        print(f"[SMS] {country.name} 当前库存 {count_text(aggregate_count)} 低于阈值 {threshold}，进入运营商二次选择")
...
def resolve_authorization_sms_selection(args: argparse.Namespace, flow_label: str = "流程", flow_key: str = "") -> dict[str, object] | None:
    env = read_env_keys(AUTH_ROOT / ".env")
    sms_enabled_value = flow_env_value(env, flow_key, "SMS_ENABLED")
    if not env_bool(sms_enabled_value, default=True):
        if flow_key.upper() in {"FREE", "CURSOR"}:
            env_name = f"{flow_key.upper()}_SMS_ENABLED"
            print(f"[SMS] 手机接码已关闭，{flow_label}无法继续，请开启 {env_name}。")
            return None
        print(f"[SMS] 手机接码已关闭，{flow_label}保持原逻辑：遇到手机号必填页会弃置账号。")
        return None
    ...
    if not api_key:
        if flow_key.upper() in {"FREE", "CURSOR"}:
            print(f"[SMS] 缺少 {api_key_name}，{flow_label}无法继续。")
            return None
    ...
    if provider_name == "grizzly":
        ...
        if flow_key.upper() == "CURSOR":
            service = (
                getattr(args, "hero_sms_service", "")
                or env.get("CURSOR_SMS_SERVICE")
                or env.get("HERO_SMS_SERVICE")
                or "ot"
            ).strip() or "ot"
            forced_country = (
                getattr(args, "country", "")
                or env.get("CURSOR_SMS_COUNTRY")
                or env.get("CURSOR_SMS_COUNTRY_SELECT")
                or env.get("HERO_SMS_COUNTRY_SELECT")
                or "England"
            ).strip()
            prompt_country = False
    ...
    try:
        if provider_name == "fivesim":
            # 5sim 娌℃湁鍥藉 ID 浣撶郴锛岀洿鎺ョ敤鏈湴 catalog + 浠锋牸鐭╅樀鍗冲彲
            priced = provider.list_country_prices(service, catalog)
            if not priced:
                raise RuntimeError("5sim did not return any country prices (service mismatch or zero balance)")
        else:
            ...
            if top_api_rows:
                ...
                try:
                    ...
                    print(f"[SMS] HeroSMS 瀹屾暣浠锋牸琛ㄦ媺鍙栧け璐ワ紝鍥藉閫夋嫨浠呬娇鐢?Top 鍒楄〃: {exc}")
            else:
                priced = provider.list_country_prices(service, countries)
            if not priced:
                raise RuntimeError("鏈В鏋愬嚭浠讳綍鍙敤鍥藉鎶ヤ环")
        if provider_name == "grizzly":
        ...
        print_country_price_table(top_rows, title=f"[SMS] {provider_label} 鏈€渚垮疁鍥藉 Top 鍒楄〃", country_id_label=f"{provider_label} ID")
        default_country = pick_country_by_price_rank(priced)
        selected_country = match_country(forced_country, priced) if forced_country else None
        if selected_country:
            print(f"[SMS] 宸查€氳繃 --country 鎸囧畾鍥藉: {selected_country.name} (+{selected_country.dial_code})锛屼环鏍?{price_text(selected_country.price)}")
            ...
            if prompt_country:
                print(f"[SMS] 褰撳墠涓嶆槸浜や簰缁堢锛岃嚜鍔ㄤ娇鐢ㄩ粯璁ゅ浗瀹? {default_country.name} (+{default_country.dial_code})")
        ...
        print(f"[SMS] 已选择国家: {selected_country.name} (+{selected_country.dial_code}), "
...
def build_auth_command(record: dict[str, str], account_file: Path, output_root: Path, sms_selection: dict[str, object] | None = None) -> list[str]:
    ...
    if sms_selection:
        ...
        if isinstance(country, PhoneCountry):
            ...
            # 5sim 闇€瑕佽嫳鏂?slug锛堜緥濡?"indonesia"锛?
...
def run_one(record: dict[str, str], index: int, total: int, output_root: Path, sms_selection: dict[str, object] | None = None) -> tuple[bool, str]:
    ...
    log(f"[鎺堟潈 {index}/{total}] 寮€濮? {record['account']}")
    env = os.environ.copy()
    ...
    status = "鎴愬姛" if ok else f"澶辫触(code={final_returncode})"
    log(f"[鎺堟潈 {index}/{total}] {status}: {record['account']}")
...
def interactive_authorize(args: argparse.Namespace | None = None) -> int:
    ...
    upload_ready = "閰嶇疆瀹屾暣" if upload_url_ok and upload_key_ok else "缂哄皯 URL/API_KEY"
    print()
    print("流程三：账号授权")
    print_table(
        ["项目", "当前值"],
    ...
    if not getattr(sys, "frozen", False) and not AUTH_SCRIPT.exists():
        print(f"[error] 鎵句笉鍒版巿鏉冭剼鏈? {AUTH_SCRIPT}")
        return 1
    if not records:
    ...
    if raw_count is None:
        count = ask_int("璇疯緭鍏ヨ繖娆¤鎺堟潈鍑犱釜璐﹀彿", default=min(1, len(records)), min_value=1, max_value=len(records))
    else:
        count = int(raw_count)
    ...
    if raw_workers is None:
        workers = ask_int("璇疯緭鍏ュ苟鍙戠嚎绋嬫暟", default=1, min_value=1, max_value=count)
    else:
    ...
    if removed_phone_required:
        ...
        log(f"已移除手机号必填弃置账号: {removed_phone_required} 个；记录: {discarded_path}")
    ...
    if removed_no_valid_org:
        discarded_path = append_discarded_accounts(no_valid_org_selected, "授权阶段 no_valid_organizations 重试失败", output_root)
        log(f"已移除 no_valid_organizations 弃置账号: {removed_no_valid_org} 个；记录: {discarded_path}")
    ...
    if removed_invalid_state:
        discarded_path = append_discarded_accounts(invalid_state_selected, "授权阶段 invalid_state 累计 2 次", output_root)
        log(f"已移除 invalid_state 弃置账号: {removed_invalid_state} 个；记录: {discarded_path}")
...
def main() -> int:
    ...
    parser.add_argument("--country", default="", help="指定国家：序号 / ISO / 接码平台国家 ID")
    ...
    parser.add_argument("--hero-sms-country-top-n", type=int, help="列出便宜国家数量，默认 10")
    parser.add_argument("--hero-sms-operator-threshold", type=int, help="低于库存阈值时二次选择运营商，默认 20")
    parser.add_argument("--grizzly-api-key", default="", help="GrizzlySMS API Key")
    parser.add_argument("--grizzly-service", default="", help="GrizzlySMS 服务代码，默认 auto")
    parser.add_argument("--grizzly-country-top-n", type=int, help="列出便宜国家数量，默认 10")
    parser.add_argument("--grizzly-provider-threshold", type=int, help="低于库存阈值时二次选择服务商，默认 20")
